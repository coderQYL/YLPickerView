package com.qyl.pickerview.builder;

import android.content.Context;
import android.view.View;
import android.view.ViewGroup;
import androidx.annotation.ColorInt;
import com.qyl.pickerview.R;
import com.qyl.pickerview.configure.PickerOptions;
import com.qyl.pickerview.listener.CustomListener;
import com.qyl.pickerview.listener.OnTimeSelectChangeListener;
import com.qyl.pickerview.listener.OnTimeSelectListener;
import com.qyl.pickerview.view.TimePickerView;
import com.qyl.pickerview.view.WheelView;
...
public class TimePickerBuilder {
     ...
     * 设置起始时间
     */
    public TimePickerBuilder setStartDate(Calendar startDate) {
        mPickerOptions.startDate = startDate;
        return this;
    }
    ...
    public TimePickerView build() {
        ...
        if (null == mPickerOptions.startDate) {
        ...
        endDate.set(Calendar.getInstance().get(Calendar.YEAR) +99,11,31);
        mPickerOptions.endDate = endDate;

        if (null == mPickerOptions.date) {
            mPickerOptions.date = Calendar.getInstance();
        }

        mPickerOptions.textSizeContent = 24;//滚轮文字大小
        mPickerOptions.textSizeSubmitCancel = 18;//确定和取消文字大小

        mPickerOptions.x_offset_year = 50;
        mPickerOptions.x_offset_month = -50;
        //如果显示“日”，则“月”不展示弧度
        if (mPickerOptions.type[2]) {
            mPickerOptions.x_offset_month = 0;
            mPickerOptions.x_offset_day = -50;
        }

//        mPickerOptions.textColorCancel = (Color.parseColor("#5D78E8"));//确定按钮文字颜色
//        mPickerOptions.textColorConfirm = (Color.parseColor("#323C47"));//取消按钮文字颜色

        mPickerOptions.textColorCancel = (mPickerOptions.context.getResources().getColor(R.color.pickerview_btnConfirmColor));//确定按钮文字颜色
        mPickerOptions.textColorConfirm = (mPickerOptions.context.getResources().getColor(R.color.pickerview_btnCancelColor));//取消按钮文字颜色
